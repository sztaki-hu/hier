...
class SimSimEnvV2:
    def __init__(self,config):
        ...
        self.env_params = self.config["environment"]["params"]
    ...
    def step(self,a):
        if self.action_space == "pick_and_place_2d":
            ...
            for i in range(self.task_params[0] + self.task_params[1]):
                ...
                if np.allclose(a[:2], block[:2], rtol=self.env_params["pick"]["rtol"], atol=self.env_params["pick"]["atol"], equal_nan=False):
                    xy_noise = np.random.normal(loc=self.env_params["place"]["mean"], scale=self.env_params["place"]["std"], size=2)
                    self.observation[(i+1)*3:(i+2)*3] = np.array([a[2]+xy_noise[0],a[3]+xy_noise[1],self.block_on_desk_z + self.subgoal_level * self.block_size])
                    break
            ...
            for i in range(self.task_params[0] + self.task_params[1]):
                block = self.observation[(i+1)*3:(i+2)*3]
                if np.allclose(a[:3], block, rtol=self.env_params["pick"]["rtol"], atol=self.env_params["pick"]["atol"], equal_nan=False):
                    self.observation[(i+1)*3:(i+2)*3] = a[3:6]
                    xy_noise = np.random.normal(loc=self.env_params["place"]["mean"], scale=self.env_params["place"]["std"], size=2)
                    self.observation[(i+1)*3:(i+2)*3][3] += xy_noise[0]
                    self.observation[(i+1)*3:(i+2)*3][4] += xy_noise[1]
                    break
            o = self.observation.copy()
        # Get reward
    ...
    def reward_shaping_subgoal_stack_blocks(self,o):
        ...
        for i in range(self.subgoal_level+1):
            ...
            for block in blocks:
                if np.allclose(target, block, rtol=self.env_params["reward"]["rtol"], atol=self.env_params["reward"]["atol"], equal_nan=False):
