...
parser.add_argument("--outdir", default="plots_runtime" ,help="Path of the output folder")
...
for taskname in taskname_list:
    for alg in algs:
        ...
        exps = []
        #exp = {"exp_name": "_".join([plotid, taskname , alg,'sparse','noher','nohl','noper','nocl']) , "seed_num":seednum}
        exps.append({"exp_name": "_".join(['1017_A', 'Panda'+taskname+'-v3',alg,'sparse','final','fix','fix','noper','controldiscreteadaptive']) , "seed_num":seednum, "color": "darkgreen", "plot_name": "HER+HiER(fix)+CL"})
        exps.append({"exp_name": "_".join(['1017_A', 'Panda'+taskname+'-v3',alg,'sparse','final','fix','prioritized','noper','controldiscreteadaptive']) , "seed_num":seednum, "color": "deepskyblue", "plot_name": "HER+PHiER(fix)+CL"})
