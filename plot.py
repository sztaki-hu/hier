...
parser.add_argument("--title", default="Success rate" ,help="Title of the plots")
...
for i in range(len(exps)):
    ...
    for j in range(exps[i]['seed_num']):
        path = os.path.join(current_dir, "logs",exps[i]['exp_name'],str(j),'runs','csv','eval_success_rate.csv')
        print(path)
...
figname = args.plotid + '_success_rate.png'
...
figname = args.plotid + '_success_rate_std.png'
plt.savefig(os.path.join(current_dir, args.outdir, figname),bbox_inches='tight')
plt.show()
